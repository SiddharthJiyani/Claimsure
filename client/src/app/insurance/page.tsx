...
export default function InsuranceDashboardPage() {
  ...
  const stats = useMemo(
    () => ({
      ...
      action: cases.filter((claim) =>
        ["ACTION_REQUIRED", "AWAITING_REVIEW", "APPEAL_READY"].includes(claim.status),
      ).length,
  ...
  const queue = cases.filter((claim) => !["RESOLVED", "CLOSED"].includes(claim.status));

  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <PageHeader
        eyebrow="Healthcare operations"
        title="Prior-auth & denial desk"
        description={`${profile?.full_name ?? "Reviewer"} — org-assigned cases only. Parse denials, run the agent, and approve citation-backed appeals.`}
      />

      <div className="grid gap-3 md:grid-cols-3">
        {[
          ["Queue", "Incoming claims from patients in your organization."],
          ["Analyze", "Trigger the agent to retrieve policy and compute evidence gaps."],
          ["Decide", "Approve, reject, or escalate the appeal draft."],
        ].map(([title, copy]) => (
          <div key={title} className="cs-panel rounded-2xl p-4">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <StatCard label="Needs review" value={stats.action} icon={AlertTriangle} />
      ...
      <section>
        ...
        {queue.length === 0 ? (
          <EmptyState
            title="No open cases"
            description="Cases appear here when a patient in your organization submits a denied or pending service. This queue is empty until then."
          />
        ) : (
          <div className="grid gap-3">
            {queue.map((claim) => (
              ...
              >
                <CaseCard claim={claim} href={`/insurance/cases/${claim.id}`} subtitle="Org-assigned claim" />
                <button
                  type="button"
                ...
                >
                  <Bot size={16} />
                  {busyId === claim.id ? "Queuing…" : "Run denial agent"}
                </button>
              </div>
            ))}
          </div>
