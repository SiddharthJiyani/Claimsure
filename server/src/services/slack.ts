...
export async function sendApprovalRequest(params: ApprovalMessageParams): Promise<{ ts: string }> {
  if (env.DRY_RUN) {
  ...
  try {
          ...
          elements: [
            {
              ...
              value: JSON.stringify({ caseId: params.caseId, action: 'approved' }),
            },
            {
              type: 'button',
              ...
              value: JSON.stringify({ caseId: params.caseId, action: 'rejected' }),
            },
            {
              type: 'button',
              text: { type: 'plain_text', text: '📋 More Info Needed' },
              action_id: 'more_info',
              value: JSON.stringify({ caseId: params.caseId, action: 'more_info' }),
            },
          ],
        },
      ],
    ...
    logger.info('Slack approval message sent', { caseId: params.caseId, ts: res.ts });
...
export async function sendEscalationAlert(params: EscalationMessageParams): Promise<void> {
  if (env.DRY_RUN) {
...
export async function sendTextMessage(channelId: string, text: string): Promise<void> {
  if (env.DRY_RUN) {
    logger.debug('DRY_RUN: sendTextMessage', { channelId, text: text.slice(0, 50) });
    return;
  }
