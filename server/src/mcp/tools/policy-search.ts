  ...
  server.tool(
    ...
    "Search payer policy documents using semantic similarity. Returns matching clauses with citation IDs.",
    {
      query: z
        ...
        .min(1)
        .describe(
          "Natural language query about payer requirements or coverage criteria",
        ),
      payer_id: z
        .string()
        .optional()
        .describe('Payer identifier to scope the search (e.g. "payer_a")'),
      service_code: z
        .string()
        .optional()
        .describe("CPT or procedure code to filter relevant policies"),
    },
    async ({ query, payer_id, service_code }) => {
      try {
            ...
            content: [
              {
                ...
                text: JSON.stringify({
                  ...
                  results: [],
                  message:
                    "No matching policy clauses found. Agent should abstain rather than guess.",
                }),
              },
            ],
          };
        ...
        logger.debug("MCP policy_search completed", {
          query,
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify({
                error: "Policy search unavailable",
                found: false,
                results: [],
              }),
            },
