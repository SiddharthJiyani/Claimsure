...
import type { CreateAppealInput, UpdateAppealInput } from '../validators/appeals.validator.js';
...
export async function getAppeal(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (user.role === 'patient' && caseData.patient_id !== user.id) throw new ForbiddenError();
...
export async function createNewAppeal(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const caseData = await getCaseById(caseId);
    if (caseData.insurer_org_id !== user.organization_id) throw new ForbiddenError();
...
export async function updateAppeal(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const caseStatusMap: Record<string, 'SUBMITTED' | 'AWAITING_REVIEW' | 'CLOSED'> = {
      APPROVED: 'SUBMITTED',
