...
export async function getCaseDetail(
  ...
  try {
    ...
    const caseData = caseWithDetails as {
      patient_id: string;
  ...
  try {
    ...
    if (user.role !== "insurance_provider") {
      throw new ForbiddenError(
        "Only insurance providers can update case status",
      );
    }
...
export async function processCase(
  ...
  try {
    ...
    if (user.role !== "insurance_provider") {
      throw new ForbiddenError(
        "Only insurance providers can trigger AI agent processing",
      );
    }
    ...
    await createAuditLog({
      ...
      metadata: {
        ...
        dry_run: body.dry_run || env.DRY_RUN,
      },
    });
...
export async function getCaseAudit(
  ...
  try {
    ...
    if (
      user.role === "insurance_provider" &&
      existing.insurer_org_id !== user.organization_id
    ) {
      throw new ForbiddenError();
    }
