...
export async function getEval(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const user = req.user!;

    if (user.role !== 'insurance_provider') {
      throw new ForbiddenError('Evaluation results are only available to insurance providers');
    }
